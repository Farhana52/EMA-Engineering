...
export default function ParticularsRenderer({ text, className = '' }: ParticularsRendererProps) {
  ...
  return (
    <div className={`text-slate-900 text-xs leading-normal space-y-1 ${className}`}>
      {rawLines.map((line, index) => {
        ...
        if (index === 0 && isFirstLineHeading) {
          ...
          return (
            <div
              key={index}
              className="font-bold text-slate-900 text-xs leading-snug mb-1"
            >
              {displayTitle}
            </div>
          );
        ...
        if (bulletMatch) {
          ...
          return (
            <div key={index} className="flex items-start space-x-1.5 pl-0.5 text-[10.5px]">
              <span className="text-neutral-500 font-bold select-none leading-none mt-0.5">•</span>
              <div className="leading-snug flex-1">
                {colonMatch ? (
                  <>
                    <span className="font-semibold text-slate-900">{colonMatch[1]} </span>
                    <span className="text-slate-700 font-normal">{colonMatch[2]}</span>
                  </>
                ) : (
                  <span className="text-slate-700 font-normal">{content}</span>
                )}
              </div>
            </div>
          );
        ...
        if (keyValMatch) {
          ...
          return (
            <div key={index} className="text-[10.5px] leading-snug">
              <span className="font-semibold text-slate-900">{keyLabel} </span>
              <span className="text-slate-700 font-normal">{valText}</span>
            </div>
          );
        }
        ...
        // Regular text line
        return (
          <div key={index} className="text-[10.5px] text-slate-700 font-normal leading-snug">
            {line}
          </div>
        );
      })}
